...
from scraper.s16_metadata import DRIVER_PATH
from scraper.s16_metadata import IMG_SRC_CSS, HREF_CSS, PRODUCT_TITLE_CSS, PRICE_CSS, SELLER_CSS, NEXT_ARROW_BTN_CSS
...
def build_driver():
    ...
    driver.get(f"https://s.1688.com/selloffer/offer_search.htm?keywords={keyword[count]}&n=y&netType=1%2C11%2C16&spm=a260k.dacugeneral.search.0")
...
def df_to_csv():
    df = pd.DataFrame(
        {"detected_date": df_detected_time, "img_src": df_img_src, "href": df_href, "product_title": df_title,
         "price_value": df_price, "seller_info": df_seller})
